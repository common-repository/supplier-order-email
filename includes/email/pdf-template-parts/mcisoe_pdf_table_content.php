...
class McisoePdfTableContent
{
    ...
    public function __construct( $options, $product_sku, $product_name, $product_qty, $product_ean, $product_price, $product_attributes, $product_meta, $product_cost_format, $product_shortdesc, $item = null, $special_meta = null, $item_data = null )
    {
        ...
        $items_template = '
                <tr>
                <td style="border: 1px solid #d9d9d9; padding: 12px; text-align: center; vertical-align: middle; white-space:nowrap;">
                <span class="text" style="font-weight:bold;">' . $product_qty . '</span>
                </td>
                <td style="border: 1px solid #d9d9d9; padding: 12px; text-align: left; vertical-align: middle;">
                <span class="text" style="font-weight:bold;">' . $product_sku . '</span>
                </td>';
        if ( $options->show_product_img == '1' ) {
            $items_template .= '<td style="border: 1px solid #d9d9d9; padding: 12px; text-align: left; vertical-align: middle; position:relative; text-align:center; white-space:nowrap;">
                                <img src="' . esc_url( $item_data['product_img'] ) . '" width="100%"
                                style="max-width:' . esc_attr( $options->product_img_width ) . 'px;">
                                </td>';
        }
        $items_template .= '<td style="border: 1px solid #d9d9d9; padding: 12px; text-align: left; vertical-align: middle; ">
        ...
        if ( $options->show_ean == '1' ) {
            $items_template .= '<td style="border: 1px solid #d9d9d9; padding:12px; text-align:left; vertical-align:middle; font-size:12px;">
        ...
        if ( $options->show_weight == '1' ) {
            ...
            $items_template .= '<td style="border: 1px solid #d9d9d9; padding:12px; text-align:left; vertical-align:middle; font-size:12px; white-space:nowrap;">
                          <span class="text">' . $item_data['product_weight'] . ' ' . $mcisoe_unit_weight . '</span>
                          </td>';
        }
        if ( $options->show_cost_prices == '1' ) {
            $items_template .= '<td style="border: 1px solid #d9d9d9; padding:12px; text-align:center; vertical-align:middle; font-size:12px; white-space:nowrap;">
